...
class BridgeManagerRpcServer(object):
    def __init__(self, host, port, user_name, password, queue_name):
        ...
        self.connection_ = pika.BlockingConnection(pika.ConnectionParameters(host=host, port=port, credentials=credentials))
...
bridge_mgr_rpc_client = BridgeManagerRpcServer('localhost', 5672, 'guest', 'guest', 'rpc_queue')
bridge_mgr_rpc_client.start()